...
char auth[] = "TMF-CRmP9gVt5I3jeyRgVCUHZ0Ssm-U0";
...
float pitch1, yaw1, roll1, pitch2, yaw2, roll2;                   // absolute orientations
float extensionAngle, deviationAngle; // joint angles
...
{  
  ...
  // #TEST override requirement for new data interrupt  
// If intPin1 goes high, either all data registers have new data
   if(1){//intFlag1 == true) {   // On interrupt, read data
...
// #TEST override requirement for new data interrupt  
   ...
   if(1){//intFlag2 == true) {   // On interrupt, read data
...
//    Serial.print(lin_az2*1000.0f, 2);  Serial.println(" mg");
    ...
    // Calculate extension angle (positive in extension, neg in flexion)
    extensionAngle = pitch2 - pitch1;
    Serial.print("Extension Angle = "); Serial.print(extensionAngle, 2); Serial.println(" deg");

    // Calculate ulnar / radial deviation angle (positive in ulnar / outward deviation)
    deviationAngle = yaw2 - yaw1;
    Serial.print("Deviation Angle = "); Serial.print(deviationAngle, 2); Serial.println(" deg");   
