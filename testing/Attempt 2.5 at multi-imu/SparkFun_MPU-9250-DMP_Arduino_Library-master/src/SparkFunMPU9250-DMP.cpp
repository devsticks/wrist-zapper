...
MPU9250_DMP::MPU9250_DMP()
{
	...
	address = 0x68; 	  // updated after address is set
}

inv_error_t MPU9250_DMP::begin(unsigned char addr)
{
	address = addr;
	...
	mpu_set_bypass(1, address); // Place all slaves (including compass) on primary bus
...
{
	return set_int_enable(enable, address);
}
...
{
	return mpu_set_int_latched(enable, address);
}

short MPU9250_DMP::getIntStatus(void)
{
	short status;
	if (mpu_get_int_status(&status, address) == INV_SUCCESS)
	{
		return status;
	}
	return 0;
...
{
	return mpu_lp_accel_mode(rate, address);
}

inv_error_t MPU9250_DMP::setGyroFSR(unsigned short fsr)
{
	inv_error_t err;
	err = mpu_set_gyro_fsr(fsr, address);
	if (err == INV_SUCCESS)
	{
		_gSense = getGyroSens();
	}
...
{
	inv_error_t err;
	err = mpu_set_accel_fsr(fsr, address);
	if (err == INV_SUCCESS)
	{
		_aSense = getAccelSens();
	}
...
{
	return mpu_set_lpf(lpf, address);
}
...
{
    return mpu_set_sample_rate(rate, address);
}
...
{
	return mpu_set_compass_sample_rate(rate, address);
}
...
{
	return mpu_configure_fifo(sensors, address);
}

inv_error_t MPU9250_DMP::resetFifo(void)
{
	return mpu_reset_fifo(address);
}
...
unsigned short MPU9250_DMP::fifoAvailable(void)
{
	...
	if (mpu_read_reg(MPU9250_FIFO_COUNTH, &fifoH, address) != INV_SUCCESS)
		return 0;
	if (mpu_read_reg(MPU9250_FIFO_COUNTL, &fifoL, address) != INV_SUCCESS)
		return 0;
...
inv_error_t MPU9250_DMP::updateFifo(void)
{
	...
	if (mpu_read_fifo(gyro, accel, &timestamp, &sensors, &more, address) != INV_SUCCESS)
		return INV_ERROR;
...
{
	return mpu_set_sensors(sensors, address);
}
...
bool MPU9250_DMP::dataReady()
{
	...
	if (mpu_read_reg(MPU9250_INT_STATUS, &intStatusReg, address) == INV_SUCCESS)
	{
...
int MPU9250_DMP::updateAccel(void)
{
	...
	if (mpu_get_accel_reg(data, &time, address))
	{
		return INV_ERROR;		
	}
	ax = data[X_AXIS];
...
int MPU9250_DMP::updateGyro(void)
{
	...
	if (mpu_get_gyro_reg(data, &time, address))
	{
		return INV_ERROR;		
	}
	gx = data[X_AXIS];
...
int MPU9250_DMP::updateCompass(void)
{
	...
	if (mpu_get_compass_reg(data, &time, address))
	{
		return INV_ERROR;		
	}
	mx = data[X_AXIS];
...
{
	return mpu_get_temperature(&temperature, &time, address);
}

int MPU9250_DMP::selfTest(unsigned char debug)
{
	long gyro[3], accel[3];
	return mpu_run_self_test(gyro, accel, address);
}
...
inv_error_t MPU9250_DMP::dmpBegin(unsigned short features, unsigned short fifoRate)
{
	...
	if (feat & DMP_FEATURE_LP_QUAT)
	{
		...
		dmp_enable_lp_quat(1, address);
	}
	else if (feat & DMP_FEATURE_6X_LP_QUAT)
		dmp_enable_6x_lp_quat(1, address);
	
	if (feat & DMP_FEATURE_GYRO_CAL)
		dmp_enable_gyro_cal(1, address);
	...
	return mpu_set_dmp_state(1, address);
}

void MPU9250_DMP::setAddr(unsigned char addr) 
{
	mpu_set_addr(addr);
}
...
{
	return dmp_load_motion_driver_firmware(address);
}
...
inv_error_t MPU9250_DMP::dmpSetFifoRate(unsigned short rate)
{
	...
	return dmp_set_fifo_rate(rate, address);
}
...
inv_error_t MPU9250_DMP::dmpUpdateFifo(void)
{
	...
	if (dmp_read_fifo(gyro, accel, quat, &timestamp, &sensors, &more, address)
		   != INV_SUCCESS)
    {
	   return INV_ERROR;
    }
...
inv_error_t MPU9250_DMP::dmpEnableFeatures(unsigned short mask)
{
	...
	return dmp_enable_feature(enMask, address);
}
        ...
        unsigned char taps, unsigned short tapTime, unsigned short tapMulti)
{
	...
	if (xThresh > 0)
	{
		...
		if (dmp_set_tap_thresh(1<<X_AXIS, xThresh, address) != INV_SUCCESS)
			return INV_ERROR;
	}
	if (yThresh > 0)
	{
		...
		if (dmp_set_tap_thresh(1<<Y_AXIS, yThresh, address) != INV_SUCCESS)
			return INV_ERROR;
	}
	if (zThresh > 0)
	{
		...
		if (dmp_set_tap_thresh(1<<Z_AXIS, zThresh, address) != INV_SUCCESS)
			return INV_ERROR;
	}
	if (dmp_set_tap_axes(axes, address) != INV_SUCCESS)
		return INV_ERROR;
	if (dmp_set_tap_count(taps, address) != INV_SUCCESS)
		return INV_ERROR;
	if (dmp_set_tap_time(tapTime, address) != INV_SUCCESS)
		return INV_ERROR;
	if (dmp_set_tap_time_multi(tapMulti, address) != INV_SUCCESS)
		return INV_ERROR;
...
inv_error_t MPU9250_DMP::dmpSetOrientation(const signed char * orientationMatrix)
{
	...
	return dmp_set_orientation(scalar, address);
}
...
inv_error_t MPU9250_DMP::dmpEnable3Quat(void)
{
	...
	return dmp_enable_lp_quat(1, address);
}
	
unsigned long MPU9250_DMP::dmpGetPedometerSteps(void)
{
	unsigned long steps;
	if (dmp_get_pedometer_step_count(&steps, address) == INV_SUCCESS)
	{
		return steps;
	}
	return 0;
...
{
	return dmp_set_pedometer_step_count(steps, address);
}

unsigned long MPU9250_DMP::dmpGetPedometerTime(void)
{
	unsigned long walkTime;
	if (dmp_get_pedometer_walk_time(&walkTime, address) == INV_SUCCESS)
	{
		return walkTime;
	}
	return 0;
...
{
	return dmp_set_pedometer_walk_time(time, address);
}
